...
public class AskActivity extends AppCompatActivity {
    ...
    private void init(Bundle state) {
        if (state != null) {
            permissions = state.getStringArray(com.rohan.app.ask.Constants.PERMISSIONS);
            rationaleMessages = state.getStringArray(com.rohan.app.ask.Constants.RATIONAL_MESSAGES);
            requestId = state.getInt(com.rohan.app.ask.Constants.REQUEST_ID);
        } else {
            Intent intent = getIntent();
            permissions = intent.getStringArrayExtra(com.rohan.app.ask.Constants.PERMISSIONS);
            requestId = intent.getExtras().getInt(com.rohan.app.ask.Constants.REQUEST_ID, 0);
        }
    }

    @Override
    public void onSaveInstanceState(Bundle state) {
        state.putStringArray(com.rohan.app.ask.Constants.PERMISSIONS, permissions);
        state.putStringArray(com.rohan.app.ask.Constants.RATIONAL_MESSAGES, rationaleMessages);
        state.putInt(com.rohan.app.ask.Constants.REQUEST_ID, requestId);
    }
    ...
    private void broadcast(String[] permissions, int[] grantResults) {
        if (grantResults.length > 0) {
            ...
            intent.setAction("com.rohan.app.ask.PERMISSION_RESULT_INTENT");
            intent.putExtra(com.rohan.app.ask.Constants.PERMISSIONS, permissions);
            intent.putExtra(com.rohan.app.ask.Constants.GRANT_RESULTS, grantResults);
            intent.putExtra(com.rohan.app.ask.Constants.REQUEST_ID, requestId);
